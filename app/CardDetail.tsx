        ...
        replace: (domNode) => {
            if (domNode.type === 'tag' && domNode.name === 'img') {
                ...
                if (!addedImages.has(src)) {
                    addedImages.add(src);
                    return <img src={src} alt={domNode.attribs.alt} className="content-image" />;
                }
                return null;
            }
            if (domNode.type === 'tag' && domNode.name === 'p') {
                return <p className="content-text">{domNode.children.map(child => (child as unknown as Text).data).join('')}</p>;
            }
        }
    });
